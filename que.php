...
ob_start();
##########
$token = "7605400256:AAHjsnB-4-5LlLv6uTcnB5CWanAFkPBWjtg"; // توكنك
$arthur = "7605400256:AAHjsnB-4-5LlLv6uTcnB5CWanAFkPBWjtg";// توكنك

$buy = "tahikal";
$userbot ="mikesecbot";
$sudo = "7601607055"; // ايديك
$admin = "$sudo";
$idBot = "7601607055";// ايدي بوتك
$admmm = $sudo;
$ADMIN = $sudo;
$Dev = array("$sudo","00000");// ايديك
$Devd = "$sudo";
$tbbots = mikesecbot;/// يوزر بوتك بدون @
 
$channel = "Mwote";/// يوزر قناتك بدون@
$chinnl = "$channel";
$DevUser = "@$buy"; // 
$Devch = "@$buy"; //  
$Userbot = "$tbbots"; // 
...
────────────────
⌯⁞ CH ⋙ **[ᴛᴇᴀᴍ arthr](https://t.me/Mwote)** 💥
⌯⁞ CH ⋙ **[Ͳᴇᴀᴍ arthur](t.me/mwote)** 🇾🇪
────────────
...
if($text == "تفعيل" or $text == "حظر" or $text == "ايدي" or $text == "كتم" or $text == "تقيد" or $text == "الاوامر" or $text == "الاعدادات" or $text == "رتبتي" or $text == "كشف" or $text == "الرتبه" or $text == "رتبته" or $text == "اضف رد" or $text == "حذف رد" or $text == "تاق" or $text == "حذف امر" or $text == "اضف امر" or $text == "تاق للكل" or $text == "/start" or $text == "الاوامر" or $text == "م1" or $text == "م2" or $text == "م3" or $text == "م4" or $text == "م5" or $text == "اوامر التحشيش"){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer)) {if ($tc == 'group' | $tc == 'supergroup'){
$add = $settings["information"]["added"];
if ($add == true){
if($setchannel == "الاشتراك الاجباري مفعل"){
$join = file_get_contents("https://api.telegram.org/bot".$arthur."/getChatMember?chat_id=@$setch&user_id=".$from_id);
...
// tag
elseif($text =="قفل التاق" or $text =="قفل الهاش تاق"){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$carlos) or in_array($from_id,$eri) or in_array($from_id,$nazar)) {$add = $settings["information"]["added"];
if ($add == true) {
bot('sendmessage',[
'chat_id'=>$chat_id,
...
elseif($text =="فتح التاق" or $text =="فتح الهاش تاق"){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$carlos) or in_array($from_id,$eri) or in_array($from_id,$nazar)) {$add = $settings["information"]["added"];
if ($add == true) {
bot('sendmessage',[
'chat_id'=>$chat_id,
...
// ردود عامة
if($text == "السعودية"){
bot('sendMessage',[ 'chat_id'=>$chat_id, 'text'=>"تاج العرب المملكة، فدوه لترابها 🇸🇦💚", 'reply_to_message_id'=>$message->message_id, ]);
}

if($text == "فدوه"){
bot('sendMessage',[ 'chat_id'=>$chat_id, 'text'=>"فداك الكون يا بعد حيّي 🤍", 'reply_to_message_id'=>$message->message_id, ]);
}

if($text == "احبك"){
bot('sendMessage',[ 'chat_id'=>$chat_id, 'text'=>"استح على وجهك يا بعدي 😂❤️", 'reply_to_message_id'=>$message->message_id, ]);
}

if($text == "@$buy"){
bot('sendMessage',[ 'chat_id'=>$chat_id, 'text'=>"⌯ هذا معرف المطور عبدالله – نورت ✨", 'reply_to_message_id'=>$message->message_id, ]);
}

if($text == "باي"){
bot('sendMessage',[ 'chat_id'=>$chat_id, 'text'=>"وين رايح؟ القروب يظلم بدونك 😢", 'reply_to_message_id'=>$message->message_id, ]);
}

if($text == "السلام عليكم" || $text == "سلام"){
bot('sendMessage',[ 'chat_id'=>$chat_id, 'text'=>"وعليكم السلام ورحمة الله وبركاته 🌹", 'reply_to_message_id'=>$message->message_id, ]);
}

if($text == "هلاو" || $text == "هلو" || $text == "هلوو"){
bot('sendMessage',[ 'chat_id'=>$chat_id, 'text'=>"هلا بك يا وجه الخير 🌟", 'reply_to_message_id'=>$message->message_id, ]);
}

if($text == "نورت"){
bot('sendMessage',[ 'chat_id'=>$chat_id, 'text'=>"نورك سابق يا بعد قلبي ✨", 'reply_to_message_id'=>$message->message_id, ]);
}

if($text == "تمام"){
bot('sendMessage',[ 'chat_id'=>$chat_id, 'text'=>"الله يديمها عليك يالغالي 🤍", 'reply_to_message_id'=>$message->message_id, ]);
}

if($text == "رابط حذف" or $text == "رابط الحذف" or $text == "اريد احذف الحساب" or $text == "ححذف"){
bot('sendMessage',[ 'chat_id'=>$chat_id,
  'text'=>"🔗 هذا رابط حذف حسابك من تيليجرام:\n[اضغط هنا](https://telegram.org/deactivate)\n\n⚠️ تنبيه: بعد الحذف ما تقدر ترجع حسابك، الله يوفقك وين ما رحت 🤍\n\n– فريق آرثر",
  'parse_mode'=>"MARKDOWN",
  'reply_to_message_id'=>$message->message_id,
]);
}

if(strpos($text,"وش رايك في")!== false){
$lockcmd = $settings["lock"]["jooki"];
if ($lockcmd == "مقفول") {
$armof = array(
"أشوفه كفو بس أحيان يخبص 😂",
"فخم، يعجبني صراحة 🔥",
"من الناس اللي ترفع الراس 👏",
"عاد لا تفتح المواضيع، خلها مستورة 🤭",
"كفو وراعي واجب دايمًا 🤍"
);
$b = array_rand($armof,1);
bot('sendmessage',[ 'chat_id'=>$chat_id, 'text'=>"$armof[$b]", 'reply_to_message_id'=>$message->message_id, ]);
}}

// الترقيات
$roles = [
  'وزير' => '📌 تمت ترقيته لمنصب *وزير القروب*، عطوه وجه 😎',
  'ملك' => '👑 تمت ترقيته إلى *ملك القروب*، تاج راسنا والله 🔥',
  'زوجتي' => '👰‍♀️ صارت *زوجتك رسميًا*، مبروك الزواج يا بعدي 💍',
  'زوجي' => '🤵‍♂️ صار *زوجك الشرعي*، الله يوفقكم 🫶',
  'مطي' => '🐎 تمت ترقيته إلى *مطي القروب*، عطوه عربه 😂',
  'كلب' => '🐶 تم تعيينه *كلب القروب* رسميًا، نباحه واصل 😂',
  'طلي' => '🐑 صار *طلي القروب*، نذبحه ولا نخليه؟ 😅',
  'ثور' => '🐃 تم رفعه *ثور حراث القروب*، خله يشيل الثقل 😂',
  'زاحف' => '🦎 صار *زاحف رسمي*، خلوا البنات يحذرونه 😂',
];

foreach ($roles as $title => $reply) {
  if ($text == "رفع $title" || $text == "ارتقاء $title") {
    if($settings["lock"]["jooki"] == "مقفول"){
      bot('SendMessage', [
        'chat_id'=>$chat_id,
        'text'=>"👤 [$re_name](tg://user?id=$re_id) $reply",
        'parse_mode'=>'markdown',
        'reply_to_message_id'=>$message->message_id
      ]);
    }
  }
  if ($text == "تنزيل $title" || $text == "حذف $title") {
    if($settings["lock"]["jooki"] == "مقفول"){
      bot('SendMessage', [
        'chat_id'=>$chat_id,
        'text'=>"👤 [$re_name](tg://user?id=$re_id) تم تنزيله من رتبة *$title*، نزل مستواه شوي 😂",
        'parse_mode'=>'markdown',
        'reply_to_message_id'=>$message->message_id
      ]);
    }
...
if ($text == "⌯ تغير اسم البوت" and in_array($from_id,$Dev)) {
  ...
  bot("sendMessage", [
    "chat_id" => $chat_id,
    "text" => "🔧 ارسل اسم البوت الجديد يا مطور",
    'parse_mode' => "MARKDOWN",
    'reply_to_message_id' => $message_id
  ]);
}
...
$botproxre = array(
  "🤖 اسمي الحين $namebot، لا تنساه!",
  "🔁 رجعت لحركاتي القديمة، اسمي $namebot",
  "✨ ناديني $namebot، ولا تقول بس بوت!",
  "👋 هلا فيك، أنا $namebot",
  "📛 اسمي الرسمي: $namebot"
);
$reproxbot = array_rand($botproxre, 1);

if (($text == "بوت" || $text == "اسم البوت" || $text == "البوت شنو اسمه" || $text == "شسمه البوت" || $text == "البوت شسمه") && $namebot == NULL) {
  if ($tc == 'group' || $tc == 'supergroup') {
    bot('sendMessage', [
      'chat_id' => $chat_id,
      'text' => "$botproxre[$reproxbot]",
      'parse_mode' => "MARKDOWN",
      'reply_to_message_id' => $message_id
    ]);
  }
...
$botre = array(
  "👂 أسمعك يالغالي، وش تبي؟",
  "💡 أنا حاضر، أمرني يا بعدي",
  "🧠 البوت شغال، تراك بنورك",
  "📶 متصل وجاهز للخدمة"
);
...
$namere = array(
  "😅 ياخي كل شوي تناديني؟",
  "📛 اسمي واضح، لا تكثر نداء",
  "😂 خلاص عرفناك عرفناك",
  "🌟 الله يذكرك بالخير، اسمي $namebot"
);
$rename = array_rand($namere, 1);

if ($text == "$namebot" && $namebot != NULL) {
  if ($tc == 'group' || $tc == 'supergroup') {
    bot('sendMessage', [
      'chat_id' => $chat_id,
      'text' => $namere[$rename],
...
$proxre = array(
  "🦾 تحت أمرك، آرثر معك!",
  "📢 كلي آذان صاغية، آرثر يسمعك",
  "💪 اطلب وتدلل، آرثر بالخدمة",
  "🕌 سبحان الله وبحمده، سبحان الله العظيم"
);
$reprox = array_rand($proxre, 1);

if ($text == "آرثر" && $namebot == NULL) {
  if ($tc == 'group' || $tc == 'supergroup') {
    bot('sendMessage', [
      'chat_id' => $chat_id,
      'text' => $proxre[$reprox],
...
##########
if ($text =="تفعيل التفلان" ){
if($status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer) || in_array($from_id,$carlos) || in_array($from_id,$eri) || in_array($from_id,$nazar)) {$add = $settings["information"]["added"];
if ($add == true) { 
 bot('sendmessage',[
 'chat_id'=>$chat_id,
 'text'=>"✵ تم تفعيل امر التفلان بنجاح
⌯ بواسطة ⋙「 [$first_name](tg://user?id=$from_id) 」
",'parse_mode'=>"markdown",
'disable_web_page_preview'=>true,
  'reply_to_message_id'=>$message_id,
...
elseif($text =="تعطيل التفلان"){
if($status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer) || in_array($from_id,$carlos) || in_array($from_id,$eri) || in_array($from_id,$nazar)) {$add = $settings["information"]["added"];
if ($add == true) {
 bot('sendmessage',[
 'chat_id'=>$chat_id,
 'text'=>"✶ تم تعطيل امر التفلان بنجاح 
⌯ بواسطة ⋙「 [$first_name](tg://user?id=$from_id) 」
",'parse_mode'=>"markdown",
'disable_web_page_preview'=>true,
  'reply_to_message_id'=>$message_id,
...
bot('sendMessage',[ 'chat_id'=>$chat_id, 'text'=>"حاضر ستادي هسه شبعه التفلان😻🤘🏿", 'reply_to_message_id'=>$message->message_id, ]);
...
'text'=>"
✶ عزيزي ⋙ [$first_name](tg://user?id=$from_id)
⌯ امر التفلان معطل من قبل الادارة
",
'parse_mode'=>"Markdown",
'reply_to_message_id'=>$message->message_id,
]);
...
'reply_markup'=>$inlinebutton,
 ...
 // الرد على أوامر "اهينه" بصيغة سعودية
 $henare = array(
   "😒 اسكت يا عسل لا أكبرك وأهينك قدام العالم!",
   "😎 ترى تراكمت عليك، اسحب نفسك لا أشقك بالكلام!",
   "🤨 لا تكثر، وجهك ما يتحمل المزح الثقيل!",
   "😂 أضحك الله يسعدك، بس اسكت قبل لا تنصقع!",
   "😤 خلاص لا تعصبني، خلك عاقل ولا تبغى مشاكل؟"
 );
 $rehena = array_rand($henare, 1);
 
 if ($replyid && !in_array($re_id, $Dev)) {
   if (in_array($text, ["اهينه", "بعد اهينه", "اهينه بعد", "اهينها", "اهينهه", "رزله", "رزلها", "رزلهه"])) {
     if ($settings["lock"]["iiirrt"] == "مقفول") {
       bot('sendMessage', [
         'chat_id' => $chat_id,
         'text' => "أبشر تم يا ذيب ✌️",
         'reply_to_message_id' => $message->message_id
       ]);
       bot('sendMessage', [
         'chat_id' => $chat_id,
 ...
 if ($replyid && in_array($re_id, $Dev)) {
   if (in_array($text, ["اهينه", "بعد اهينه", "اهينه بعد", "اهينها", "اهينهه", "رزله", "رزلها", "رزلهه"])) {
     if ($settings["lock"]["iiirrt"] == "مقفول") {
       bot('sendMessage', [
         'chat_id' => $chat_id,
         'text' => "😳 وش تقول؟ تبي أهين مطورك اللي مربيك؟ مستحيل يا بعدي 😤",
         'parse_mode' => "MARKDOWN",
         'reply_to_message_id' => $message->message_id,
         'disable_web_page_preview' => true
       ]);
 ...
 if ($replyid && in_array($re_id, $eri)) {
   if (in_array($text, ["اهينه", "بعد اهينه", "اهينه بعد", "اهينها", "اهينهه", "رزله", "رزلها", "رزلهه"])) {
     if ($settings["lock"]["iiirrt"] == "مقفول") {
       bot('sendMessage', [
         'chat_id' => $chat_id,
         'text' => "😤 على مين تهين؟ هذا مبرمج البوت يا بعد راسي! ✋",
         'parse_mode' => "MARKDOWN",
         'reply_to_message_id' => $message->message_id,
         'disable_web_page_preview' => true
       ]);
 ...
 if (in_array($text, ["اهينه", "بعد اهينه", "اهينه بعد", "اهينها", "اهينهه", "رزله", "رزلها", "رزلهه"])) {
   if ($settings["lock"]["iiirrt"] == "مفتوح") {
     bot('sendmessage', [
       'chat_id' => $chat_id,
       'text' => "✶ أمر (اهينه) مقفل حاليًا 🚫\n⌯ تم من قبل ⋙「 [$first_name](tg://user?id=$from_id) 」",
       'parse_mode' => "Markdown",
       'reply_to_message_id' => $message->message_id
     ]);
   }
 ...
 // رد مباشر على الكلمات بدون ريباي
 if (in_array($text, ["اهنه", "اهينه", "اهينك", "اهينه اهينه", "اهينها"])) {
   bot('sendMessage', [
     'chat_id' => $chat_id,
     'text' => "👊 لا تتحمس واجد، تراك بتاكلها لو زودتها!",
     'reply_to_message_id' => $message->message_id
   ]);
 }
...
$arthur = file_get_contents("kindi.txt");
...
bot("sendmessage",['chat_id'=>$chat_id,
'text'=>"Welcome To Source arthur \n    
✵⁞ [Arthur Team](https://t.me/Mwote)
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
⌯︙[Source Channel](https://t.me/Mwote)\n
⌯︙[Source Install](https://t.me/Mwote)\n
⌯︙[Changing ID Channel](https://t.me/Mwote)\n
⌯︙[TEAM Source](https://t.me/Mwote)*\n
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
⌯︙[ConnectDeV](https://t.me/tahikal)
",'parse_mode'=>"Markdown",
'disable_web_page_preview'=> true ,
'reply_to_message_id'=>$message_id,
'reply_markup'=>json_encode([
'inline_keyboard'=>[
[['text'=>"- تنصيب سورس",'url'=>"https://telegram.me/mikesecbot?start"]],
